...
macro_rules! impl_then_tuple {
  ($th:ident $ph:ident) => {
    ...
    where
      $ph: Parser<'a, $th>
    {
      fn parse(&mut self, input: &'a [Token]) -> Result<(($th,), &'a [Token]), String> {
        let ($ph,) = self;
...
pub trait Parser<'a, T> {
  fn parse(&mut self, input: &'a [Token]) -> Result<(T, &'a [Token]), String>;
  fn and_then<P>(&mut self, mut other: P) -> impl FnMut(&'a [Token]) -> Result<(T, &'a [Token]), String>
  where
    P: Parser<'a, T>
  {
    move |input: &'a [Token]| {
...
impl<'a> Parser<'a, Token> for Token {
...
where
  F: FnMut(&'a [Token]) -> Result<(T, &'a [Token]), String>,
{
  fn parse(&mut self, input: &'a [Token]) -> Result<(T, &'a [Token]), String> {
    self(input)
  }
...
macro_rules! choice_trait_impl {
  ...
  ($ph:ident $($p:ident)*) => {
      ...
      $($p: Parser<'a, T>),+
    {
      fn choice(&mut self, input: &'a [Token]) -> Result<(T, &'a [Token]), String> {
        match self.0.parse(input) {
          Err(_) => choice_trait_inner!(1, self, input, $ph $($p)+),
          res => res
        }
      }
    }
...
pub fn alt<'a, T, L>(mut parsers: L) -> impl FnMut(&'a [Token]) -> Result<(T, &'a [Token]), String>
where
  L: Choice<'a, T>
{
...
pub fn opt<'a, T, P>(mut parser: P) -> impl FnMut(&'a [Token]) -> Result<(Option<T>, &'a [Token]), String>
where
  P: Parser<'a, T>
{
  move |input: &'a [Token]| {
...
pub fn seperated_list<'a, T, U, P1, P2>(mut first: P1, mut second: P2) -> impl FnMut(&'a [Token]) -> Result<(Vec<T>, &'a [Token]), String>
where 
  P1: Parser<'a, T>,
  P2: Parser<'a, U>
{
