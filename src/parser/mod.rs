...
use framework::{alt, delimited, many, map, opt, preceded, seperated_list, terminated, Parser as ParserTrait};
...
impl<'a> Parser<'a> {
  ...
  fn parse_stmt(tokens: &'a [Token]) -> Result<(Stmt, &'a [Token]), ParserError> {
    let (stmt, tokens) = alt((
      ...
      map(terminated(TokenKind::Break, TokenKind::Semi), |_| Stmt::Break),
      map(terminated(TokenKind::Continue, TokenKind::Semi), |_| Stmt::Continue),
      map(terminated(Self::parse_expr, TokenKind::Semi), |expr| Stmt::Expr(expr)),
    ))(tokens)?;
  ...
  fn parse_if_stmt(tokens: &'a [Token]) -> Result<(Stmt, &'a [Token]), ParserError> {
    ...
    let (cond, tokens) = delimited(TokenKind::OpenParen, Self::parse_expr, TokenKind::CloseParen)(tokens)?;
  ...
  fn parse_expr(tokens: &[Token]) -> Result<(Expr, &[Token]), ParserError> {
    ...
    while let Some(token) = tokens.first() {
      let Token { kind, line, col, pos  } = token;
      cur_col = *col;
      cur_line = *line;
      cur_pos = *pos;
      match kind {
        TokenKind::Identifier(name) => {
          if tokens.len() > 1 && tokens[1].kind == TokenKind::OpenParen {
            ...
            let (args, rest) = delimited(TokenKind::OpenParen, seperated_list(Self::parse_expr, TokenKind::Comma), TokenKind::CloseParen)(tokens)?;
        ...
        TokenKind::IntLiteral(n) => output.push(Expr(ExprKind::Literal(LiteralValue::Integer(*n)),RefCell::new(Some(Type::Int)))), // TODO: Handle typing
        TokenKind::FloatLiteral(f) => output.push(Expr(ExprKind::Literal(LiteralValue::Float(*f)), RefCell::new(Some(Type::Float)))), // TODO: Handle typing
        TokenKind::BoolLiteral(b) => output.push(Expr(ExprKind::Literal(LiteralValue::Boolean(*b)), RefCell::new(Some(Type::Boolean)))), // TODO: Handle typing
        _ if token.kind.op_info().is_some() => {
          ...
          if op_info.is_unary {
            ...
            while let Some(&top_op) = op_stack.last() {
              let top_info = top_op.kind.op_info().expect("Token at top of stack is not an operator");
